...
async function startBot() {
  try {
    ...
    client.on("authenticated", async (session) => {
      console.log("✅ Authenticated!");
      await saveSession(session); // Save session after successful authentication
    });
    ...
    client.on("message", async (message) => {
      ...
      // Handle message commands (jokes, AI responses, image generation, etc.)
      if (text.startsWith("/joke")) {
        try {
          const joke = await getJoke();
          await message.reply(`🤣 *Here's a joke for you:*\n\n🟢 ${joke}`);
          ...
          console.error("❌ Error fetching joke:", error.message);
          await message.reply(
            "❌ Unable to fetch a joke. Please try again later."
          );
        }
      }
      ...
      if (text.startsWith("image")) {
        const prompt = text
          ...
          .trim();
        if (!prompt) {
          await message.reply("❌ Please provide a valid image description!");
          return;
        }
      ...
      let response;
      try {
        response = await chatWithGemini(text);
      } catch (error) {
        console.error("❌ AI Response Error:", error.message);
        response = "⚠️ Sorry, I couldn't understand that. Please try again!";
      }
